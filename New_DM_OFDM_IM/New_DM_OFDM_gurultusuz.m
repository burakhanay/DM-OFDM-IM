...
for kk=1:100%100 kere dönüp hata sýfýrdan farklýysa ekrana yansýtacak!!
    ...
    s_1D = reshape(s.',1,[]);
    %norm(s_1D(:))^2/length(s_1D(:))
    outputIFFT = (nFFTSize/sqrt(nFFTSize))*ifft(fftshift(s_1D),nFFTSize);
    enerji_esitleyen = 1/sqrt(mean(abs(outputIFFT).^2));
    outputIFFT =outputIFFT*enerji_esitleyen;
    %norm(outputIFFT(:))^2/length(outputIFFT(:));
    ...
    rx_signal = (sqrt(nFFTSize)/nFFTSize)*fftshift(fft(outputIFFT,nFFTSize));
    rx_signal = reshape(rx_signal,n,g).'/enerji_esitleyen;
    ...
    if not(hata==0)
        disp(hata);%hata sýfýrdan farklý olursa yazdýrýyor.
    end
end
